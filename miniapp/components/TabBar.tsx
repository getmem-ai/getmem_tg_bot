...
export function TabBar({ tabs, active, onChange }: TabBarProps) {
  return (
    <nav
      className="fixed inset-x-0 bottom-0 z-30 border-t border-black/[0.07] dark:border-white/[0.09] bg-tg-bg/85 backdrop-blur-xl pb-safe"
      aria-label="Primary"
    >
            ...
            >
              <span
                className={`flex h-7 w-12 items-center justify-center rounded-full transition ${
                  isActive ? "bg-tg-button/12 text-tg-button" : "text-tg-hint"
                }`}
              >
                <Icon
